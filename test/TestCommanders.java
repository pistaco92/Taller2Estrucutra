...
public class TestCommanders {
    ...
    public void testSorroundings() {
        ...
        Commanders.FichaSorroundings comand = Commanders.FichaSorroundings.getBuildier().setFicha(ficha).createWithTablet(values);
	    command.execute();

	// error 
	    if (command.direccion == "horizontal" && command.itsSorrounded == true) {
		assert true;
	} else {
		assert false;
	}
    ...
    public void testDirecction() {
        ...
        Ficha ficha = Ficha.getBuildier().setPlayer("A").setRow(1).setColumn(2).create();
	Commander.FichaDireccion comand  = Commanders.FichaDireccion.getBuildier().setFicha(ficha).setTablet(values).setDireccion("horizontal").create();
	command.execute();
    ...
    public void testAllDirecciotion() {
        ...
        Ficha ficha = Ficha.getBuildier().setPlayer("A").setRow(1).setColumn(2).create();
	Commander.FichaDireccionMultiple comands  = Commanders.FichaDireccion.getBuildier().setTablet(values).setFicha(ficha).createInAllDirecction();
    ...
    public void testConect() {
	    ...
	    assert comand.oneconnect == true && check[0];
